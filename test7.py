...
实现方法：
  ...
  语音合成 → Qwen3-TTS（CustomVoice Vivian 声线），生成/播放双线程流水线
...
依赖库（pip install）：
  ...
  qwen-tts torch soundfile — Qwen3-TTS 语音合成（本地 GPU 推理）
"""
...
# 强制禁用sox，用ffmpeg/torchaudio做音频处理
...
# ============================================================
# TTS 播放（Qwen3-TTS CustomVoice，流水线：生成 + 播放分离，消除句间停顿）
# ============================================================
...
print("加载 Qwen3-TTS 语音模型...")
qwen_tts_model = Qwen3TTSModel.from_pretrained(
    QWEN_TTS_MODEL_PATH,
    device_map="cuda:0",
    dtype=torch.float32,
)
print("Qwen3-TTS 语音模型加载完成\n")

_tts_queue = queue.Queue()       # 待合成的文本
_play_queue = queue.Queue()      # 已生成的 (wav, sr, text, duration) 待播放

# 共享状态：当前正在播放的句子信息（供 GUI 动画同步语音节奏）
_tts_current_text = ""
_tts_current_duration = 0.0
_tts_current_start = 0.0
_tts_playing = False             # 是否正在播放（替代 pygame.mixer.music.get_busy）
_tts_lock = threading.Lock()


def _tts_generator():
    """生成线程：从 _tts_queue 取文本 → Qwen3-TTS → 放入 _play_queue"""
    while True:
        text = _tts_queue.get()
        if text is None:
            ...
            break
        try:
            audio_wav, sr = qwen_tts_model.generate_custom_voice(
                text=text,
            ...
            wav = audio_wav[0]
            duration = len(wav) / sr
            _play_queue.put((wav, sr, text, duration))
...
def _tts_player():
    """播放线程：从 _play_queue 取 wav → sounddevice 播放"""
    global _tts_playing
    while True:
        item = _play_queue.get()
        if item is None:
            break
        wav, sr, text, duration = item
        try:
            # 共享状态 → GUI 动画同步
            with _tts_lock:
